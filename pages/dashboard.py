...
active_count, pending_tasks, total_courses = 1, 0, 0
...
for u in active_users_list:
    name = u.get('full_name') or 'Student'
    avatar = u.get('avatar_url')
    
    if avatar:
        avatar_div = f'<img src="{avatar}" class="active-avatar">'
    else:
        initials = "".join([n[0] for n in name.split()[:2]]).upper() if name else "ST"
...
st.markdown('<div class="sh-dashboard">Student Dashboard</div>', unsafe_allow_html=True)
...
st.markdown(f"""
    <div class="welcome-banner">
        ...
        <div class="welcome-subtitle">Here is what is happening in your classes today. Stay updated with your pending tasks and active course materials.</div>
    </div>
""", unsafe_allow_html=True)
...
with col4:
    st.markdown(f"""
        <div class="metric-card" style="border-top-color: #4285F4;">
            <div class="metric-label">Your Course</div>
...
with sum_col3:
    ...
    if not recent_courses:
        ...
        for c in recent_courses:
            # Safely grab a name or title depending on your courses table schema
            c_name = c.get('course_name', c.get('title', c.get('name', 'Course')))
...
st.markdown("""
    <div class="about-card">
        ...
        <div class="about-text">
            ...
            <br><br>
            Use the navigation menu on the left to access your personalized <b>Schedule</b>, track upcoming assignments in the <b>To-Do List</b>, download reviewers from the <b>Courses</b> repository, and collaborate with your peers in the <b>Post/Share</b> discussion board. All system updates and critical deadlines pushed by your Class Representative will appear in your <b>Notifications</b> feed.
        </div>
    </div>
""", unsafe_allow_html=True)